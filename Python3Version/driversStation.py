...
def sendThreshold(newThr):
    ...
    print(s)
    broadcastSocket.sendto(str.encode(s),('255.255.255.255',broadcastPort))
    broadcastSocket.sendto(str.encode(s),(piAddr,broadcastPort))
    broadcastSocket.sendto(str.encode('showThreshold=5'),('255.255.255.255',broadcastPort))
    broadcastSocket.sendto(str.encode('showThreshold=5'),(piAddr,broadcastPort))
    ...
    while True:
        ...
        try:
            ...
            if (deltaBroadcast > 1.0):
                lastBroadcast = time.clock()
                broadcastSocket.sendto(str.encode(BroadcastData),('255.255.255.255',broadcastPort))
                broadcastSocket.sendto(str.encode(BroadcastData),(piAddr,broadcastPort))
                
                # DEBUG ONLY 
                #broadcastSocket.sendto(str.encode('robot138'),('255.255.255.255',broadcastPort))
                #broadcastSocket.sendto(str.encode('robot138'),(piAddr,broadcastPort))
            ...
            if (ch == ord('o')):
                print("Targeting On")
                broadcastSocket.sendto(str.encode('targetOn'),('255.255.255.255',broadcastPort))
                broadcastSocket.sendto(str.encode('targetOn'),(piAddr,broadcastPort))
                                
            elif (ch == ord('f')):
                print("Targeting Off")
                broadcastSocket.sendto(str.encode('targetOff'),('255.255.255.255',broadcastPort))
                broadcastSocket.sendto(str.encode('targetOff'),(piAddr,broadcastPort))
                
            elif (ch == ord('t')):
                print("Threshold Display")
                broadcastSocket.sendto(str.encode('showThreshold=08'),('255.255.255.255',broadcastPort))
                broadcastSocket.sendto(str.encode('showThreshold=08'),(piAddr,broadcastPort))
