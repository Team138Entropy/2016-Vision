...
#        rpt[tgtName] = t
    ...
    if (reportSocket != None):
        reportSocket.sendto(str.encode(msg), (UDP_IP, UDP_PORT))
...
def vline(img,color,offset,thickness):
    ...
    height,width,colors = img.shape
    point1 = ((int)(offset),0)
    point2 = ((int)(offset),(int)(height-1))
    cv2.line(img,point1,point2,color,thickness)
    
def hline(img,color,offset,length,thickness):
     
    height,width,colors = img.shape
    point1 = ((int)(width/2-length/2),(int)(offset))
    point2 = ((int)(width/2+length/2),(int)(offset))
    cv2.line(img,point1,point2,color,thickness)
